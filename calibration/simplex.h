// slightly modified on Nov 2, 2015
// slightly modified on Oct 2, 2016
...
It measures the difference of the simplex centroid*(nN+1) of consecutive iterations.
...
namespace BT{
	...
	std::vector<D> Simplex(
		...
		int iterations=1e4)//iteration step number
	{
		int nN = init.size();  //space dimension
		//coefficients
		...
		std::vector<D> xcentroid_old(nN,0);   //simplex center * (nN+1)
		std::vector<D> xcentroid_new(nN,0);   //simplex center * (nN+1)
		std::vector<D> vf(nN+1,0);            //f evaluated at simplex vertexes       
		int x1=0;    //x1:   f(x1) = min { f(x1), f(x2)...f(x_{n+1} }
		int xn=0;    //xnp1: f(xnp1) = max { f(x1), f(x2)...f(x_{n+1} }
		int xnp1=0;  //xn:   f(xn)<f(xnp1) && f(xn)> all other f(x_i)
		...
		{ // construct the trial simplex
			...
			x.push_back(init);//x.size()=nN+1, x[i].size()=nN

			// xcentriod
			std::transform(init.begin(), init.end(), 
				xcentroid_old.begin(), std::bind2nd(std::multiplies<D>(), nN+1) );
		...
		for(cnt=0; cnt<iterations; ++cnt){
			...
			std::vector<D> xg(nN, 0);//xg: centroid of the nN best vertexes
			...
			if (diff/nN < tol){
				cout << "terminating the optimizer" << endl;;
		...
		if(cnt==iterations){//max number of iteration achieves before tol is satisfied
			std::cout<<"Iteration limit is reached, result may not be optimal"<<std::endl;
		}
		return x[x1];
	}
